        ...
        match command {
            ...
            "sys" => {
                ...
                writeln!(
                    writer,
                    "{:<15} {} {}% ({}MB / {}MB)",
                    "Memory:", bar, mem_pct, used_mem, total_mem
                )
                .unwrap_or(());
                writeln!(
                    writer,
                    ...
                    .find(|d| d.mount_point() == std::path::Path::new("/"))
                {
                    ...
                    let bar_disk = format!(
                        "[{}{}]",
                        "#".repeat(filled_disk),
                        ".".repeat(bar_len - filled_disk)
                    );
                    writeln!(
                        writer,
                        "{:<15} {} {}% ({}GB / {}GB)",
                        "Disk (/):", bar_disk, disk_pct, used, total
                    )
                    .unwrap_or(());
                }
            ...
            "help" => {
                ...
                writeln!(
                    writer,
                    "\x1b[1;32mShyell - Advanced Performance Shell\x1b[0m"
                )
                .unwrap_or(());
                writeln!(writer, "\x1b[1mSystem Performance:\x1b[0m").unwrap_or(());
                writeln!(
                    writer,
                    "  sys         Show system overview (CPU, Mem, Disk, Uptime)."
                )
                .unwrap_or(());
                writeln!(writer, "  top         Show top 10 processes by CPU usage.").unwrap_or(());
                writeln!(
                    writer,
                    "  bench <cmd> Prefix any command to measure its time/resources."
                )
                .unwrap_or(());
